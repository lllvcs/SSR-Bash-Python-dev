...
echo "##################################
      SSR-Bash-Python-dev 自检系统
##################################"
#List /usr/local
echo "############Filelist of /usr/local" >> /root/report.json
cd /usr/local
ls >> /root/report.json
#List /usr/local/ssr-bash-python-dev
echo "############Filelist of /usr/local/SSR-Bash-Python-dev" >> /root/report.json
...
if [ ! -f "/usr/local/bin/ssr" ]; then
  echo "SSR-Bash-Python-dev主文件缺失，请确认服务器是否成功连接至Github"
  echo "SSR Miss" >> /root/report.json
  exit
fi
if [ ! -f "/usr/local/SSR-Bash-Python-dev/server.sh" ]; then
  echo "SSR-Bash-Python-dev主文件缺失，请确认服务器是否成功连接至Github"
  echo "SSR Miss" >> /root/report.json
  exit
fi
